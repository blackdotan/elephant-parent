 ...
 */
public class GeneratedDomainArrayHandlerJavaFilePlugin extends PluginAdapter {

    public static final String ENUMERATION_FLAT = "DomainArray";
            ...
            for (String col : cols.split(",")) {
                ...
                if (column != null) {
                    ...
                    if ( field.getName().equals(column.getJavaProperty())) {
                        // 字段<参数>类型
                        String domain = StringUtils.easyAppend("{}.domain.{}{}", iTargetPackage, iModel, ColumnUtils.retClazzNameFormat(column));
                        FullyQualifiedJavaType iModelClass = new FullyQualifiedJavaType(domain);
                        // 字段类型
        ...
        if ( cols !=null ) {
            ...
            for (String col : cols.split(",")) {
                ...
                if (column != null) {
                    ...
                    FullyQualifiedJavaType iSupperClass = new FullyQualifiedJavaType(JsonArrayHandler.class.getSimpleName());
                    iSupperClass.addTypeArgument(iModelClass);
                    Set<FullyQualifiedJavaType> set = new HashSet<FullyQualifiedJavaType>();
                    set.add(iModelClass);
                    set.add(iSupperClass);
                    set.add(new FullyQualifiedJavaType(handler));
                    ...
                    if (isOverrideEnumeration) {
                        ...
                        // 生成Domain类
                        List<CompilationUnit> answer = Arrays.asList(
                                generator.generateDomainJavaFile(column, iModelClass, set),
                                generator.generateHandlerJavaFile(column, iModelClass, iSupperClass, set));
